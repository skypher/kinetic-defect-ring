...
$cA/n$ off its disk.  Every other fixed-index pole is separated from
the window by a positive constant on the $n\lambda$ scale, so the same
argument gives distance $c$ there.  For growing indices, the horizontal
separation increases until $q$ is bounded below, where the last
fixed-distance argument above applies.  Thus
