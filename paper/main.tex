...
Eliminate successively with the first equation at each site, whose pivot
is $b_j$.  The substituted second equations are precisely the recurrence
for the pairs $(u_{j-1}^+,u_j^-)$ displayed above.  After one circuit the
residual condition is $(M-I)(u_{n-1}^+,u_0^-)^{\mathsf T}=0$, where
$M=T_{r_{n-1}}\cdots T_{r_0}$.  More explicitly, after the $n$ pivots
$b_j$, use the coefficients $-1$ of
$u_{j+1}^-$ in $n-2$ consecutive recurrence equations as unit pivots.
The two equations left are the displayed residual system, up to
triangular row additions of determinant one.  Hence the
Schur-complement determinant is
$\pm\prod_jb_j\det(M-I)$; comparison with the leading coefficient
$+\lambda^{2n}$ fixes the sign as
$-\prod_jb_j\det(M-I)$.  Since $\det M=1$, one has
...
\end{equation}
throughout $\Omega_n$ near zero, apart from
$|\lambda-\lambda_1^+|<A n^{-3}$.  Otherwise there would be a sequence
$\lambda_n$ violating \eqref{eq:fixed-exclusion-margin}.  Pass to a
subsequence according as $\zeta_n=n^2\lambda_n$ is bounded or unbounded.
...
$w_n^2=\nu(\nu+2\alpha)+O(n^{-1})$.  Substitution of
\eqref{eq:critical-exact-coefficient} into the exact formula
\eqref{eq:finite-green-z}, with
$(1+z^n)/(1-z^n)=\coth(w_n/2)$, proves item 1.  The formula is even in
$w_n$, so the conclusion agrees on overlapping branch neighborhoods.
...
\cite[Sec.~II, Eqs.~(2.2)--(2.4)]{Schnitzer1993}.  We compare it with
the next lattice correction before introducing a point defect.
