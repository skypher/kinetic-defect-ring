...
pseudo-Green functions and Chebyshev formulas give exact propagators and
the treatment of defective trapping sites
\cite[Sec.~VI]{Giuggioli2020}.  The present problem instead asks for the full
...
\]
with $Q_{n,-}$ given by \eqref{eq:Qminus-poly}.  At $\omega=1$, define
$r_-=\operatorname{ord}_{\lambda=-2}Q_{n,-}$ at the fixed parameter pair and take
the resultant of $Q_{n,-}/(\lambda+2)^{r_-}$ instead.  The protected
exceptional parameters \eqref{eq:protected-ep} and the flat-band cases
of Theorem \ref{thm:flat-band} are supplied separately; they are not
asserted to arise from this affected-sector resultant.  Root orders and
...
a root.  At $\omega=1$ the same statement is applied to the specified
reduced polynomial.  The preceding two theorems identify the Jordan
block attached to every such root and separately account for the
protected and flat contributions.
\end{proof}
