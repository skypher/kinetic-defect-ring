...
\cite[Secs.~VI--VII and Apps.~C and E]{Giuggioli2020}.  The present problem instead asks for the full
...
\]
The fast-pole terms, the wave numbers $q_k^*\ge\eta$, and the endpoint
terms have total derivative $O(1)$.  This proves the derivative bound in
...
\]
A change of the arccosh branch replaces
$w$ by $\pm w+2\pi\mathrm i n m$.  Under the sign change both
$s=\sinh(w/n)$ and $\coth(w/2)$ change sign, while the additive term is
a period; hence $\Theta\coth(w/2)$ is unchanged.  The quantity
$\operatorname{dist}(w,2\pi\mathrm i\mathbb Z)$ is also unchanged.
Thus all subsequent estimates are branch independent.
The right-edge windows stay a fixed multiple of $n^{-1}$ to the right
of the tumble pole $-2\omega$.  Consequently $|a|$ is bounded below
and $|\Theta|$ is bounded there.  If $w$ is near zero, the apparent
singularity of the cotangent cancels: Taylor expansion in the two exact
identities gives
...
$|\lambda+2\omega|\ge c(n^{-1}+|\Im\lambda|)$.  Away from $w=0$,
\eqref{eq:finite-green-z} and \eqref{eq:coth-distance-bound} therefore
give
...
We now bound the distance in \eqref{eq:critical-green-distance-reduction}.
...
\end{equation}
shows that $w=2\pi\mathrm i\ell$ precisely at the two poles with index
$\ell$.  Suppose first that
$\theta=w-2\pi\mathrm i\ell$ has $|\theta|<1$, and choose the
representative $1\le\ell\le\lfloor n/2\rfloor$, so that
$q=\ell k/n\in(0,\pi]$; the case $\ell=0$ was handled by
\eqref{eq:critical-zero-cancellation}.  Taylor's theorem gives the
uniform upper bound
\begin{equation}\label{eq:critical-cosh-local-bound}
 |D_\ell(\lambda)|
 \le C\left(\frac{|\sin q|}{n}|\theta|
                  +\frac{|\theta|^2}{n^2}\right).
\end{equation}
For each of the finitely many simple poles inside $\Omega_{B,n}$, the
scaled map $\nu=n\lambda\mapsto w$ has a derivative bounded above and
below at the pole.  The inverse-function theorem and compactness of
the boundaries of fixed scaled neighborhoods therefore give
$|w-2\pi\mathrm i\ell|\ge cA/n$ off the deleted disk, once
$n\ge N(A,B)$.  At $\alpha=k$, write
$\Delta=\lambda-\lambda_c$.  If
$|\Delta|\ge A n^{-3/2}$, then
\[
 |D_1(\lambda)|=|\Delta^2-r_n^2|\ge cA^2n^{-3},
 \qquad r_n=O(n^{-2}),
\]
and \eqref{eq:critical-cosh-local-bound}, with
$|\sin(k/n)|\asymp n^{-1}$, gives
$|w-2\pi\mathrm i|\ge cA^2/n\ge cA/n$.

It remains to control the first pole just outside $\Omega_{B,n}$.
Choose $q_0\in(0,\pi/2)$.  For $q\le q_0$ and exterior
$\ell\ge L+1$, the two poles are conjugate for large $n$; write them
$h_{\ell,n}\pm\mathrm i t_{\ell,n}$.  Their horizontal distance from
the window is
\[
 u_{\ell,n}=1-\cos(\ell k/n)-B/n^2.
\]
The midpoint definition of $B$ and the expansion
$1-\cos q=q^2/2+O(q^4)$ show first that
$u_{L+1,n}\ge c\sqrt B\,n^{-2}$ and then, by monotonicity of
$1-\cos q$ on $[0,q_0]$, that the same lower bound holds for every
such exterior index.  Moreover
$t_{\ell,n}=\sqrt{\sin^2q-\alpha^2/n^2}\ge c q$ because
$\ell\ge L+1\ge2$ and $\alpha\le k$.  For $u\ge0$ and real $y,t$,
\begin{equation}\label{eq:critical-conjugate-product-bound}
 |(u+\mathrm i(y-t))(u+\mathrm i(y+t))|\ge2u|t|;
\end{equation}
this follows by squaring both sides and completing the square in
$y^2$.  Consequently the right side of
\eqref{eq:critical-w-pole-factorization} is at least
$c u_{\ell,n}q$.  Since $q\ge2k/n$ and $|\theta|<1$,
\eqref{eq:critical-cosh-local-bound} is at most
$Cq|\theta|/n$.  Hence
$|\theta|\ge cn u_{\ell,n}\ge c\sqrt B/n$.

For $q\in[q_0,\pi]$, both homogeneous poles have real part at most
$-c(q_0)<0$, whereas the window boundary tends to zero.  Thus
$|D_\ell(\lambda)|\ge c(q_0)^2$.  This contradicts
\eqref{eq:critical-cosh-local-bound} when $|\theta|<1$ and $n$ is
large, so in this range the distance is at least one.  We have proved
\[
 \operatorname{dist}(w,2\pi\mathrm i\Z)
 \ge c\min\{1,A/n,\sqrt B/n\}
\]
outside the deleted disks.  Combining this with
\eqref{eq:critical-green-distance-reduction} proves item 1.

For $\alpha>k$, the choice $d_1<d<d_2$ leaves only the slow first pole
in $\Omega_{d,n}$.  The scaled inverse-function argument gives distance
$cA/n$ off its disk.  Choose an integer $L_0$ with $L_0k>2\alpha$.
For each fixed $2\le\ell\le L_0$, both pole factors have horizontal
distance at least $c/n$ from the window.
The factorization \eqref{eq:critical-w-pole-factorization} and the local
bound \eqref{eq:critical-cosh-local-bound} give a lower bound
$c n^{-2}$ and, when $|\theta|<1$, an upper bound
$C|\theta|n^{-2}$ for $|D_\ell(\lambda)|$.  Thus
$|w-2\pi\mathrm i\ell|=|\theta|\ge c$; this argument also covers a
possible coalescence $\alpha=\ell k$.

For $\ell>L_0$ and $q=\ell k/n\le q_0$, the two poles are conjugate,
with $t_{\ell,n}\ge cq$.  If $h_{\ell,n}$ is their common real part,
then every $\lambda\in\Omega_{d,n}$ has horizontal separation
\[
 \Re\lambda-h_{\ell,n}
 \ge u_{\ell,n}:=\frac{\alpha-d}{n}+1-\cos q.
\]
Equation \eqref{eq:critical-conjugate-product-bound} therefore gives
$|D_\ell(\lambda)|\ge c u_{\ell,n}q$.  If
$\theta=w-2\pi\mathrm i\ell$ and $|\theta|<1$, then
\eqref{eq:critical-cosh-local-bound}, together with $q\ge L_0k/n$,
gives $|D_\ell(\lambda)|\le Cq|\theta|/n$.  Hence
\[
 |\theta|\ge cnu_{\ell,n}\ge c(\alpha-d).
\]
For $q\in[q_0,\pi]$, the fixed horizontal separation used in item 1
again gives distance at least one.  Thus
$\operatorname{dist}(w,2\pi\mathrm i\Z)\ge c\min\{1,A/n\}$, and
equation
\eqref{eq:critical-green-distance-reduction} proves item 2.
\end{proof}
...
eigenvalue outside the cluster
satisfies
...
\ref{lem:critical-green-off-pole} with that $R$.  If
$\alpha\le k$, choose a midpoint value $B$ from
Lemma~\ref{lem:critical-green-off-pole}, first so large that the
protected first root lies strictly inside $\Omega_{B,n}$ and then so that
$2|b|C/\sqrt B<1/8$; choose $A_0$ so that
$2|b|C_B/A_0<1/8$.  If $\alpha>k$, use the window $\Omega_{d,n}$ from
item 2 of that lemma and choose $A_0$ so that
$2|b|C_d/A_0<1/4$.  In either case, increase $A_0$ further, if
necessary, so that the zeros in all the simple-pole and grouped
comparisons below lie strictly inside their respective disks, and fix
any $A\ge A_0$.  Finally increase $n$ until
$2|b|C_B/n<1/4$ in the first case or $2|b|C_d/n<1/4$ in the second.
Since $2\delta=2b/n$, the two
off-pole estimates give in either case
...
gives exactly two affected roots in the grouped disk.  If $b=0$, these
are the two homogeneous odd-sector roots.  This proves the asserted
root count and the independence from increasing $A$ in
