...
\cite{SadjadiMiri2008}.  The present problem instead asks for the full
...
$(\omega,\omega_0)\in K$.
\begin{enumerate}
\item For every $M<\infty$ there is $C_M$ such that, whenever
  $\lambda=\zeta/n^2$, $|\zeta|\le M$, and
  \[
...
Fix $\alpha>0$, put $\omega=\alpha/n$ and $k=2\pi$, and restrict
$\lambda$ to an arbitrary fixed disk.  The following estimates hold.
\begin{enumerate}
\item Suppose $\alpha\le k$.  For an integer $L\ge1$, set
  ...
  There are constants $C$ and $C_B$, independent of $A$ and $n$, such
  that on the remaining set, for $A\ge1$ and all sufficiently large
  $n$,
...
\item Suppose $\alpha>k$.  Define
  ...
  \end{equation}
  throughout $\Omega_{d,n}$, for $A\ge1$ and all sufficiently large
  $n$.
\end{enumerate}
\end{lemma}
...
representative $q=\ell k/n\in[0,\pi]$.  Taylor's theorem gives the
uniform upper bound
...
Fix a sufficiently large constant $A$.  If $\alpha\ne k$, the critical
first cluster consists, with algebraic multiplicity, of the protected
roots $p_{1,n}^\pm$ and the affected roots in the two disks
$|\lambda-p_{1,n}^\pm|<A n^{-2}$.  If $\alpha=k$, it consists of the
two protected roots and all affected roots in the disk centered at
$\lambda_c=\cos(k/n)-1-k/n$ given by
\[
 |\lambda-\lambda_c|<A n^{-3/2}.
\]
The root counts in the proof below show that this definition is
independent of any further increase of $A$ once $n$ is sufficiently
large.
\end{definition}
...
belongs to the critical first cluster of
Definition~\ref{def:critical-first-cluster}.  More precisely, if
...
Gershgorin's theorem confines all eigenvalues to a fixed disk.  If
...
$2|b|C/\sqrt B<1/8$.  Next choose $A$ so that
$2|b|C_B/A<1/8$, and finally increase $n$ until
$2|b|C_B/n<1/4$.  If $\alpha>k$, use the window $\Omega_{d,n}$ from
item 2 of that lemma, choose $A$ so that $2|b|C_d/A<1/4$, and then
increase $n$ so that $2|b|C_d/n<1/4$.  Since $2\delta=2b/n$, the two
...
\[
 P_{0,n}(\Delta)=\Delta^2-r_n^2
                 +\frac{4b}{n^2}(\Delta-k/n).
\]
For fixed nonzero $b$ and sufficiently large $A$, both zeros of
$P_{0,n}$ lie strictly inside the circle.  The difference between the
...
square root is required.  The affected-parity eigenvalues outside the
homogeneous telegraph spectrum are exactly the zeros of
